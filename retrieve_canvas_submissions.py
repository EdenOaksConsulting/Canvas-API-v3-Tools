...
# Import transform function from transform_canvas.py
try:
    from transform_canvas import transform_v3_to_v2
except ImportError:
...
LOG_CONFIG = {
    ...
    'file': 'retrieve_canvas_submissions.log',  # Log file name
}
...
def setup_logging(log_file=None, log_level='INFO'):
    ...
    if log_file is None:
        log_file = LOG_CONFIG.get('file', 'retrieve_canvas_submissions.log')
...
def main(username: str = None, password: str = None, bearer_token: str = None,
    ...
    try:
        ...
        if config_form_id:
            logger.info(f"Retrieving form ID {config_form_id} from API_CONFIG...")
            
            try:
                form_data = client.get_form_by_id(config_form_id)
                
                # Create filename using form ID and name
                form_name = form_data.get('name', 'Unknown').replace('/', '_').replace('\\', '_')
                filename = f"form_{config_form_id}_{form_name}.json"
                ...
                logger.info(f"Saved form {config_form_id} ({form_name}) to {filepath}")
                
            except Exception as e:
                logger.error(f"Error retrieving form {config_form_id}: {e}")
        ...
        # Retrieve each submission by ID and save to unique file
        successful = 0
        failed = 0
        transformed = 0
        transform_failed = 0
        ...
        for idx, submission_summary in enumerate(submission_list, 1):
            ...
            if not submission_id:
                logger.warning(f"Submission at index {idx} has no ID, skipping")
                failed += 1
                continue
            
            try:
                logger.info(f"Retrieving submission {idx}/{len(submission_list)}: ID {submission_id}")
                full_submission = client.get_submission_by_id(submission_id)
                
                # Create unique filename using submission ID
                ...
                if form_data and transform_v3_to_v2:
                    ...
                    logger.debug(f"Skipping transformation for submission {submission_id}: form_data not available")
                elif not transform_v3_to_v2:
                    logger.debug(f"Skipping transformation for submission {submission_id}: transform function not available")
...
Examples:
  ...
  python retrieve_canvas_submissions.py -u user@example.com -p password
  
  # Using Bearer token
  python retrieve_canvas_submissions.py --bearer-token YOUR_TOKEN
  
  # Retrieve last 14 days
  python retrieve_canvas_submissions.py -u user@example.com -p password --days 14
  
  # Filter by form ID
  python retrieve_canvas_submissions.py -u user@example.com -p password --form-id 12345
  
  # Custom output file
  python retrieve_canvas_submissions.py -u user@example.com -p password -o my_submissions.json
        """
    )
    ...
    parser.add_argument(
        ...
        dest='log_file',
        default=None,
        help='Path for log file (default: retrieve_canvas_submissions.log)'
    )
